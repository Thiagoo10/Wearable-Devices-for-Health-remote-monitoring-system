...
client.username_pw_set("tiagocaixeiro", "Hivemqcloud1234")
# connect to HiveMQ Cloud on port 8883 (default for MQTT)
client.connect("0568b8867dd7492986fd891e32d423d1.s1.eu.hivemq.cloud", 8883)
...
try:
    with open("auth_key.txt", "r") as f:
        ...
        client.publish("User_Details", payload=auth_key_from_file, qos=1)
except FileNotFoundError:
...
def heart_logger(data):
    ...
    print ('Calories: ', binfo['calories'])
    print ('Distance travelled in meters: ', binfo['meters'])
    client.publish("Steps", payload= binfo['steps'] , qos=0)
    client.publish("Fat_burned", payload= binfo['fat_burned'] , qos=0)
    client.publish("Calories", payload= binfo['calories'] , qos=0)
    client.publish("Distance", payload= binfo['meters'] , qos=0)
    client.publish("HR", payload=data, qos=0)
